    ...
    <div class="modal-content p-3 p-md-5">
      ...
      <div class="modal-body p-md-0">
        <div class="text-center mb-4">
          <h3 class="role-title mb-2 pb-0">{{ __('Add params', ['params' => 'Role']) }}</h3>
          <p>Set role permissions</p>
        </div>
        <!-- Add role form -->
        <form id="addRoleForm" class="row g-3" action="/add/role" method="POST">
          @csrf
          <div class="col-12 mb-4">
            <div class="form-floating form-floating-outline">
              <input type="text" id="modalRoleName" name="modalRoleName" class="form-control" required placeholder="Enter a role name" tabindex="-1" />
              ...
              <table class="table table-flush-spacing">
                ...
                <tr>
                  ...
                  <td>
                      ...
                      <input class="form-check-input selectAllCheckboxAddRole" type="checkbox" id="selectAllAddRole" />
          ...
          <div class="col-12 text-center">
            <button type="submit" class="btn btn-primary me-sm-3 me-1" id="btn-submit-add">Submit</button>
            <button type="reset" class="btn btn-outline-secondary btn-close-model" data-bs-dismiss="modal" aria-label="Close">Cancel</button>
          </div>
        </form>
        <!--/ Add role form -->
      </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    addRoleModal.addEventListener('show.bs.modal', function (event) {
        var checkboxes = addRoleModal.querySelectorAll('.form-check-input');
        checkboxes.forEach(function (checkbox) {
            checkbox.checked = false;
        });
    });
    addRoleModal.addEventListener('hidden.bs.modal', function (event) {
        var checkboxes = addRoleModal.querySelectorAll('.form-check-input');
        checkboxes.forEach(function (checkbox) {
            checkbox.checked = false;
        });
